...
class GAApp:
    def __init__(self, root):
        ...
        ttk.Label(ctrl, text="Селекция:").grid(row=2, column=2, sticky="w", padx=(10,0))
        ttk.Combobox(ctrl, textvariable=self.params["selection_method"],
                     values=["tournament","roulette"], width=10).grid(row=2, column=3)

        ttk.Label(ctrl, text="Вручную рёбра (0-1,0-2):").grid(row=3, column=0, columnspan=4, sticky="w", pady=(10,0))
        self.edges_input = tk.Text(ctrl, width=20, height=4)
        self.edges_input.grid(row=4, column=0, columnspan=4)

        ttk.Button(ctrl, text="Загрузить файл", command=self.load_file).grid(row=5, column=0, columnspan=2, pady=5)
        ttk.Button(ctrl, text="Инициализировать", command=self.initialize).grid(row=5, column=2, columnspan=2, pady=5)

        ttk.Button(ctrl, text="Следующий", command=self.next_step).grid(row=6, column=0, pady=5)
        ttk.Label(ctrl, text="Пропустить").grid(row=6, column=1)
        self.skip_entry = ttk.Entry(ctrl, width=5)
        self.skip_entry.insert(0, "5")
        self.skip_entry.grid(row=6, column=2)
        ttk.Button(ctrl, text="Пропустить", command=self.skip_steps).grid(row=6, column=3)

        ttk.Label(ctrl, text="Список рёбер:").grid(row=7, column=0, columnspan=4, sticky="w", pady=(10,0))
        self.edges_display = tk.Text(ctrl, width=20, height=6, state="disabled")
        self.edges_display.grid(row=8, column=0, columnspan=4)
    ...
    def load_file(self):
        ...
        with open(path, "r") as f:
            content = f.read()
        self.edges_input.delete("1.0", tk.END)
    ...
    def initialize(self):
        ...
        self.population = [self.generate_individual(n)
                           for _ in range(self.params["pop_size"].get())]
        self.best_scores.clear(); self.mean_scores.clear(); self.generation=0
        self.update_visuals()
    ...
    def step_ga(self):
        pop = self.population; eds = self.edges
        ...
        for _ in pop:
            ...
            new.append(self.mutate(c, m_rate))
        self.population = new
    ...
    def update_edges_display(self):
        ...
        self.edges_display.delete("1.0", tk.END)
        for u,v in self.edges:
            self.edges_display.insert(tk.END, f"{u} - {v}\n")
